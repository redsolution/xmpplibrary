...
do_decode(<<"os">>,
	  <<"http://xabber.com/protocol/auth-tokens">>, El,
	  Opts) ->
    decode_xabbertoken_os(<<"http://xabber.com/protocol/auth-tokens">>,
			  Opts, El);
do_decode(<<"client">>,
	  <<"http://xabber.com/protocol/auth-tokens">>, El,
	  Opts) ->
...
do_encode({xabbertoken_field, _, _, _, _, _, _} = Field,
	  TopXMLNS) ->
    encode_xabbertoken_field(Field, TopXMLNS).

do_get_name({xabbertoken_field, _, _, _, _, _, _}) ->
    <<"field">>;
...
do_get_ns({xabbertoken_field, _, _, _, _, _, _}) ->
    <<"http://xabber.com/protocol/auth-tokens#items">>;
...
pp(xabbertoken_issue, 3) -> [client, os, expire];
...
pp(xabbertoken_field, 6) ->
    [var, token, desc, ip, last, expire];
pp(_, _) -> no.

records() ->
    [{xabbertoken_issue, 3}, {xabbertoken_x_token, 3},
     {xabbertoken_revoke, 1}, {xabbertoken_query, 0},
     {xabbertoken_x_fields, 1}, {xabbertoken_field, 6}].
...
decode_xabbertoken_field_els(__TopXMLNS, __Opts,
			     [_ | _els], Ip, Desc, Last, Expire, Token) ->
    decode_xabbertoken_field_els(__TopXMLNS, __Opts, _els,
				 Ip, Desc, Last, Expire, Token).
...
encode_xabbertoken_field({xabbertoken_field, Var, Token,
    ...
    _els = lists:reverse('encode_xabbertoken_field_$ip'(Ip,
							__NewTopXMLNS,
							'encode_xabbertoken_field_$desc'(Desc,
											 __NewTopXMLNS,
...
'encode_xabbertoken_field_$desc'(undefined, __TopXMLNS,
				 _acc) ->
    _acc;
'encode_xabbertoken_field_$desc'(Desc, __TopXMLNS,
				 _acc) ->
    [encode_xabbertoken_desc_item(Desc, __TopXMLNS) | _acc].
...
decode_xabbertoken_os(__TopXMLNS, __Opts,
		      {xmlel, <<"os">>, _attrs, _els}) ->
    Cdata = decode_xabbertoken_os_els(__TopXMLNS, __Opts,
				      _els, <<>>),
    Cdata.

decode_xabbertoken_os_els(__TopXMLNS, __Opts, [],
			  Cdata) ->
    decode_xabbertoken_os_cdata(__TopXMLNS, Cdata);
decode_xabbertoken_os_els(__TopXMLNS, __Opts,
			  [{xmlcdata, _data} | _els], Cdata) ->
    decode_xabbertoken_os_els(__TopXMLNS, __Opts, _els,
			      <<Cdata/binary, _data/binary>>);
decode_xabbertoken_os_els(__TopXMLNS, __Opts,
			  [_ | _els], Cdata) ->
    decode_xabbertoken_os_els(__TopXMLNS, __Opts, _els,
...
encode_xabbertoken_os(Cdata, __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"http://xabber.com/protocol/auth-tokens">>,
				    [], __TopXMLNS),
    _els = encode_xabbertoken_os_cdata(Cdata, []),
    _attrs = xmpp_codec:enc_xmlns_attrs(__NewTopXMLNS,
					__TopXMLNS),
    {xmlel, <<"os">>, _attrs, _els}.

decode_xabbertoken_os_cdata(__TopXMLNS, <<>>) -> <<>>;
decode_xabbertoken_os_cdata(__TopXMLNS, _val) -> _val.

encode_xabbertoken_os_cdata(<<>>, _acc) -> _acc;
encode_xabbertoken_os_cdata(_val, _acc) ->
    [{xmlcdata, _val} | _acc].
...
decode_xabbertoken_issue(__TopXMLNS, __Opts,
	...
	decode_xabbertoken_issue_els(__TopXMLNS, __Opts, _els,
				     undefined, undefined, undefined),
    {xabbertoken_issue, Client, Os, Expire}.

decode_xabbertoken_issue_els(__TopXMLNS, __Opts, [], Os,
			     Client, Expire) ->
    {Os, Client, Expire};
...
decode_xabbertoken_issue_els(__TopXMLNS, __Opts,
			     [_ | _els], Os, Client, Expire) ->
    decode_xabbertoken_issue_els(__TopXMLNS, __Opts, _els,
				 Os, Client, Expire).

encode_xabbertoken_issue({xabbertoken_issue, Client, Os,
			  Expire},
			 __TopXMLNS) ->
    __NewTopXMLNS =
	xmpp_codec:choose_top_xmlns(<<"http://xabber.com/protocol/auth-tokens">>,
				    [], __TopXMLNS),
    _els = lists:reverse('encode_xabbertoken_issue_$os'(Os,
							__NewTopXMLNS,
							'encode_xabbertoken_issue_$client'(Client,
											   __NewTopXMLNS,
...
'encode_xabbertoken_issue_$os'(undefined, __TopXMLNS,
			       _acc) ->
    _acc;
'encode_xabbertoken_issue_$os'(Os, __TopXMLNS, _acc) ->
    [encode_xabbertoken_os(Os, __TopXMLNS) | _acc].
