...
do_encode({mark_markable} = Markable, TopXMLNS) ->
...
do_get_name({mark_markable}) -> <<"markable">>;
do_get_name({mark_received, _, _}) -> <<"received">>.

do_get_ns({mark_displayed, _, _}) ->
    <<"urn:xmpp:chat-markers:0">>;
do_get_ns({mark_markable}) ->
    <<"urn:xmpp:chat-markers:0">>;
do_get_ns({mark_received, _, _}) ->
    <<"urn:xmpp:chat-markers:0">>.

get_els({mark_received, _id, _sub_els}) -> _sub_els;
get_els({mark_displayed, _id, _sub_els}) -> _sub_els.

set_els({mark_received, _id, _}, _sub_els) ->
    {mark_received, _id, _sub_els};
set_els({mark_displayed, _id, _}, _sub_els) ->
    {mark_displayed, _id, _sub_els}.

pp(mark_markable, 0) -> [];
pp(mark_received, 2) -> [id, sub_els];
pp(mark_displayed, 2) -> [id, sub_els];
pp(_, _) -> no.
